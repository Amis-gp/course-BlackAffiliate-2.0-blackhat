...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!user) {
      ...
      if (registrationRequest) {
        ...
        if (registrationRequest.password === password) {
          ...
          const response = { 
            success: false, 
            message: 'Ваш акаунт ще не підтверджений адміністратором', 
            isPending: true,
          ...
          const response = { success: false, message: 'Невірний пароль для запиту на реєстрацію' };
      ...
      const response = { success: false, message: 'Користувач з таким email не знайдений' };
    ...
    if (user.password !== password) {
      ...
      const response = { success: false, message: 'Невірний пароль' };
    ...
    const response = { success: false, message: 'Помилка сервера' };
